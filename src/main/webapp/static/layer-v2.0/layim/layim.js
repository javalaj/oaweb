...
xxim.layinit = function(){
    ...
    try{
        /*
		 * if(!localStorage.layimState){ config.aniTime = 0;
		 * localStorage.layimState = 1; }
		 */
...
xxim.tabchat = function(param){
    ...
    // xxim.chatbox.find('.layim_seechatlog').attr('href', config.chatlogurl +
	// param.id);
...
xxim.searchDates = function(name){
    ...
    config.json(config.api.search, {}, function(datas){
        if(datas.status === 1){
                 ...
                 +'<ul class="xxim_chatlist" style="display: block;">';
                    for(; i < myflen; i++){
                        ...
                        if(datas.data[i].item){
                        	itemLength = datas.data[i].item.length;
                        }
                        for(var j = 0; j < itemLength; j++){
                        	if(item[j].name.indexOf(name)>=0){
                        		str += '<li data-id="'+ item[j].id +'" class="xxim_childnode" type="'+  'one' +'"><img src="';
                        		if(item[j].face){
                                	str += item[j].face;
                                }else{
                                	str += "/jeesite/userfiles/1/images/user.jpg";
                                }
                        		str += '" class="xxim_oneface"><span class="xxim_onename">'+ item[j].name +'</span></li>';
                        		num++;
                        	}
                        }
            ...
            xxim.socket.sendData("_online_user_"+currentId);
        } else {
            ...
            if(myflen > 0){
                if(index !== 2){
                    for(; i < myflen; i++){
                        ...
                        for(var j = 0; j < itemLength; j++){
                            str += '<li data-id="'+ item[j].id +'" class="xxim_childnode" type="'+ (index === 0 ? 'one' : 'group') +'"><img src="';
                            if(item[j].face){
                            	str += item[j].face;
                            }else{
                            	str+="/jeesite/userfiles/1/images/user.jpg";
                    ...
                    for(; i < myflen; i++){
                    	...
                    	for(var j = 0; j < itemLength; j++){
                            ...
                            str += '" class="xxim_oneface"><span class="xxim_onename">'+ item[j].name +'</span><em class="xxim_time">'+ item[j].time +'</em></li>';
                        }
                    }
                    str += '</ul></li>';
                }
    ...
    var xximNode = xxim.layimNode = $('<div id="xximmm" class="xxim_main">'
            ...
            +'<li class="xxim_online" id="xxim_online">'
                ...
                +'<div class="xxim_setonline">'
                    ...
                    +'<span class="xxim_setoffline"><i class="fa fa-check-circle"></i>离线</span>'
                +'</div>'
            +'</li>'
            +'<li class="xxim_mymsg" id="xxim_mymsg" title="我的私信"><i class="fa fa-comment"></i><a onclick=\'top.openTab("'+config.msgurl+'","查看所有邮件", false)\''+'></a></li>'
            +'<li class="xxim_seter" id="xxim_seter" title="设置"><i class="fa fa-gear"></i><a onclick=\'top.openTab("'+config.infourl+'","个人信息", true)\''+'></a></li>'
