  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ,'<div class="layui-layim-info">'
      ...
      ,'<div class="layui-layim-status">'
        ...
        ,'</ul>'
      ,'</div>'
      ,'<p class="layui-layim-remark" title="{{# if(d.mine.sign){ }}{{d.mine.sign}}{{# } }}"><input id="layim_user_sign_box" type="text" value="{{ d.mine.remark||d.mine.sign||"你很懒，没有写签名" }}" class="layim-sign-box layim-sign-hide"></p>'
    ,'</div>'
  ...
  var anim = 'layui-anim-up', events = {
    ...
    ,chat: function(othis){
      ...
      if(type === 'friend'){
    	//  data = cache[type][index].list[list];
          for(var i=0; i<cache[type][index].list.length; i++){
          	if(cache[type][index].list[i].id == othis.attr('id').substring(12)){
          		data = cache[type][index].list[i];
          		break;
          	}
          }
      } else if(type === 'group'){
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        ...
        ,content: cache.base.chatLog  + thatChat.data.id + '&type=' + thatChat.data.type
      });
    }
