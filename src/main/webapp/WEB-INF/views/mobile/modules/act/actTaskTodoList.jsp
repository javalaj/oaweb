		...
		<div class="team-header">
			<div style="padding:0 15px;">
				<div id="segmentedControl"
					...
					<a class="mui-control-item mui-active" id="todo">待办事项</a>
					<a class="mui-control-item" id="tosend">已发事项</a>
					<a class="mui-control-item" id="historic">已办事项</a>
				</div>
			</div>
			
			<div class="main-tit teamtask-tit">
				<span>待办事项</span> <a class="mui-pull-right" href="${mctx}/act/task/process/" >
				<span class="mui-icon mui-icon-plus"></span></a> 
				<a class="mui-pull-left" id="searchsy"><span class="mui-icon mui-icon-search"></span></a>
			</div>
			
			<div id="query" class="mui-content" style="display: none;">
				<form:form id="searchForm" modelAttribute="act" class="search-form"
				  action="${mctx}/act/task/todo/">				
			...
			<div id="daiban" class="mui-control-content  oa-contents mui-active">
				...
				<ul class="mui-table-view ">
					...
					<li class="mui-table-view-cell mui-media">
						<c:choose>
						<c:when test="${procDef.name=='日常借支申请流程' ||procDef.name=='请假流程' || procDef.name=='加班申请流程' || procDef.name=='出差申请流程' || procDef.name=='公司收文'|| procDef.name=='申请用车流程' || procDef.name=='私车公用流程' || procDef.name=='资质借用流程' || procDef.name=='印章使用流程'|| procDef.name=='外出申请流程' || procDef.name=='忘记打卡申请流程' || procDef.name=='专项借支申请流程'}">
							<a href="${mctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&commid=${fns:urlEncode(task.description)}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}" class="mui-navigate-right">
						</c:when> 
						
						<c:when test="${procDef.name=='周计划评价'}">
							 <a href="${mctx}/user/userWeekplan/viewMonthsum?sameid=${task.description}">
						</c:when>
						...
						<div class="mui-media-body">
							...
							<p class="mui-ellipsis msg-title">
								<span>${fns:abbr(not empty act.vars.map.title ? act.vars.map.title : task.id, 60)}</span>
							</p>
						</div>
						</a>
					</li>
