...
<body>
	<header class="mui-bar mui-bar-nav">
		<a href="${mctx}/user/userWeekplan/form" class=" mui-icon mui-icon-left-nav mui-pull-left"></a>
			<c:if test="${Isdraft=='0' }">
		<h1 class="mui-title">编写我的周计划</h1>
		</c:if>
		<c:if test="${Isdraft=='1' }">
		<h1 class="mui-title">查看我的周计划</h1>
		</c:if>
		
	</header>
	<div class="main divMargin">
		<form id="inputFormcz" action="${mctx}/user/userWeekplan/save"
				method="post" class="form-inline">
		<div class="heads" id="heads_js">
						<h3 style="text-align: center;font-size:17px;font-weight: 400;height: 46px;line-height: 46px;">
							<font>第${list1.weeksign }周</font>&emsp;(开始日期：
							<fmt:formatDate value="${list1.firstdate}"
								pattern="yyyy-MM-dd" />
							)
						</h3>
						<input type="hidden" value="${list1.id }" name="id" />
						<input type="hidden"
							value="<fmt:formatDate value="${list1.firstdate}"
								pattern="yyyy-MM-dd" />"
							name="firstdate" />
						<input type="hidden" name="plandate"
							value='<fmt:formatDate value="${list1.plandate}" pattern="yyyy-MM-dd"/>' />
						<input type="hidden" name="weeksign" value="${list1.weeksign }" />
		</div>
			<div class="main-content">
				...
				<div name="${now.id}">
					<div class="content" >
						<div class="content_div_type" id="content_div_type_js" >
							...
							<input type="text" style="display: none;"  id="wrid"
										name="wrid" value="${now.id}">
							<select name="beginday"
								...
								<option value="7"
									<c:if test="${now.beginday==7}">selected</c:if>>周日</option>
							</select>
						<span class="content_div_date_title"
							style="text-align: center;">至</span>
						<select name="endday"
							class="form-control" style="text-align: center;"><option value="1"
				...
				<c:if test="${Isdraft=='1' }">
							...
							<div class="content" >
								<div class="content_div_type" id="content_div_type_js" >
									<input type="text" style="display: none;"  id="wrid"
										name="wrid" value="${now.id}">
										<c:choose>
										<c:when test="${now.beginday==1}">
										周一
										</c:when>
										<c:when test="${now.beginday==2}">
										周二
										</c:when>	
										<c:when test="${now.beginday==3}">
										周三
										</c:when>	
										<c:when test="${now.beginday==4}">
										周四
										</c:when>	
										<c:when test="${now.beginday==5}">
										周五
										</c:when>
										<c:when test="${now.beginday==6}">
										周六
										</c:when>
										<c:otherwise>
										周日
									...
									<span class="content_div_date_title" style="text-align: center;">至</span>	
										<c:choose>
										<c:when test="${now.endday==1}">
										周一
										</c:when>
										<c:when test="${now.endday==2}">
										周二
										</c:when>	
										<c:when test="${now.endday==3}">
										周三
										</c:when>	
										<c:when test="${now.endday==4}">
										周四
										</c:when>	
										<c:when test="${now.endday==5}">
										周五
										</c:when>
										<c:when test="${now.endday==6}">
										周六
										</c:when>
										<c:otherwise>
										周日
					...
					<div class="content_div_content" id="content_div_content_js">
						<textarea class="content_div_content" name="content"placeholder="请输入计划内容"  readonly="readonly" rows="4">${now.content}</textarea>
					</div>
				</div>
				<div class="notsolve">
					<div class="content_div_method">
						<textarea class="content_div_content" name="deliverables"placeholder="请输入交付物"  readonly="readonly" rows="4">${now.deliverables }</textarea>
					</div>
				</div>
				<div class="content_div_select_null" id="content_div_select_null_js"></div>
				</div>
	...
	<script id="title" src="${ctxStatic}/mobile/js/Base.js"
		type="text/javascript" charset="utf-8" data="编写我的周计划"></script>

	<script src="${ctxStatic}/mobile/frame_js/mui.min.js"></script>
	<script type="text/javascript">
		mui.init();
		var line ='<div class="main-content"><input id="cz"  type="text" style="display: none;" value="0"><input type="hidden" name="delFlag" value="0"><div class="content"><div class="content_div_type" id="content_div_type_js"><span class="content_div_date_title">时间:</span><input type="hidden" name="wrid" value="null"><select name="beginday" class="form-control"><option value="1">周一</option><option value="2">周二</option><option value="3">周三</option><option value="4">周四</option><option value="5">周五</option><option value="6">周六</option><option value="7">周日</option></select><span class="content_div_date_title" style="text-align: center;">至</span><select name="endday" class="form-control"><option value="1">周一</option><option value="2">周二</option><option value="3">周三</option><option value="4">周四</option><option value="5">周五</option><option value="6">周六</option><option value="7">周日</option></select><div class="content_div_type" id="content_div_type_js"><textarea class="content_div_content" name="content"placeholder="请输入计划内容" rows="4"></textarea></div><div><div class="content_div_content" id="content_div_content_js"><div class="notsolve"><textarea class="content_div_content" name="deliverables"placeholder="请输入交付物" rows="4"></textarea></div><div class="content_div_select_null" id="content_div_select_null_js"></div></div>'
		//validate();
