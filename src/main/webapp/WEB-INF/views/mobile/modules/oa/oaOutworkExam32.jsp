...
<body>
	...
	<div class="mui-content">
		...
		<div class="divcontent">
			<form action="${mctx}/oa/oaOutwork/dealExam3" method="post"
				id="inputForm">
				...
				<input type="hidden" value="${oaOutwork.scheduleProgram}"
					name="scheduleProgram" /> <input type="hidden"
					value="${oaOutwork.examname}" name="examname" /> <input
					type="hidden" value="${oaOutwork.examtext}" name="examtext" /> <input
					type="hidden" name="sign" value="2"> <input type="hidden"
					value="${oaOutwork.examname2}" name="examname2" /> <input
					type="hidden" value="${oaOutwork.examtext2}" name="examtext2" /> <input
					type="hidden" value="${oaOutwork.fileurl}" name="fileurl" /> <input
					type="hidden" name="realEndTime"
					value="<fmt:formatDate value="${oaOutwork.realEndTime}"
					pattern="yyyy-MM-dd HH:mm:ss" />" />
				<input type="hidden" value="${oaOutwork.time}" name="time" />



				<ul class="mui-table-view divcontent">
					<li class="mui-table-view-cell"><span> 文件编号：</span> <span>${oaOutwork.fileId }</span>
					</li>
					<li class="mui-table-view-cell"><span> 部门名称：</span> <span>${oaOutwork.officeName }</span>
					</li>
					<li class="mui-table-view-cell"><span> 申请人：</span>
						<span>${oaOutwork.createBy.name }</span></li>
					<li class="mui-table-view-cell"><span> 申请时间：</span> <span><fmt:formatDate
								value="${oaOutwork.createDate }" pattern="yyyy-MM-dd HH:mm:ss" /></span>
					</li>
					<li class="mui-table-view-cell"><span> 出差任务：</span> <span>${oaOutwork.task }</span>
					</li>
					<li class="mui-table-view-cell"><span> 项目或合同：</span> <span>${oaOutwork.projectName }</span>
					</li>
					<li class="mui-table-view-cell"><span> 日程安排：</span> <span>${oaOutwork.scheduleProgram }</span>
					</li>
					<li class="mui-table-view-cell"><span> 目的地：</span>
						<span>${oaOutwork.address }</span></li>
					<li class="mui-table-view-cell"><span> 访问单位：</span> <span>${oaOutwork.unitName }</span>
					</li>
					<li class="mui-table-view-cell"><span> 拜访对象：</span> <span>${oaOutwork.unitUserName }</span>
					</li>
					...
					<li class="mui-table-view-cell"><span> 出差费用：</span> <span>${oaOutwork.cost}</span>
					</li>
					<li class="mui-table-view-cell"><span> 结束时间：</span> <span><fmt:formatDate
								value="${oaOutwork.realEndTime}" pattern="yyyy-MM-dd HH:mm:ss" /></span>
					</li>
					<li class="mui-table-view-cell"><span> 出差时长：</span> <span>${oaOutwork.time}</span>
					</li>
					<li class="mui-table-view-cell"><span> 审批意见：</span> <span style="width:58%;"><input
							type="radio" name="exam" class="exam" value="0"></input>同意 <input
							type="radio" name="exam" class="exam" checked="checked" value="1"></input>不同意</span>
					</li>
					<li class="mui-table-view-cell"><span> 审批留言：</span> <input
						class="inputs" type="text" name="examtext3"></li>
				</ul>
