...
<oam:body>
 	<oam:header>
		...
		<h1 class="mui-title">协议框架</h1>
		<contract:seg id="mySeg" activeCellName="oaContractProtocol" />
	</oam:header>
	<oam:message content="${message}"/>
	<oam:content>
		<oam:searchWidget cancelSelector="#cancelButton" paddingTop="44px">
			<oam:form id="searchForm" modelAttribute="oaContractProtocol" action="${mctx}/oa/contract/oaContractProtocol/" method="post">
				...
				<oam:listView>
					...
					<oam:listViewCell inputName="cooperationUnits" title="合作单位" cellType="input">
						<form:input path="cooperationUnits" htmlEscape="false" maxlength="64" placeholder="输入合作单位"/>
					</oam:listViewCell>
			...
			<c:choose>
				<c:when test="${page.list.size()>0}">
					<c:forEach items="${page.list}" var="oaContractProtocol" varStatus="status">
						<oam:listViewCell onclick="location.href='${mctx}/oa/contract/oaContractProtocol/form?pageType=view&id=${oaContractProtocol.id}'">
							<c:if test="${not empty oaContractProtocol.contractTime}">
								<p>
									<font color="#1a81d1">
										签订日期：<fmt:formatDate value="${oaContractProtocol.contractTime}" pattern="yyyy-MM-dd"/>
									</font>
								</p>
							</c:if>
							<p class="mui-ellipsis">${oaContractProtocol.contractName}</p>
						</oam:listViewCell>
					</c:forEach>				
				</c:when>
				<c:otherwise>
	...
	<contract:menu hideMenuCellName="oaContractProtocol" id="topPopover"/>
</oam:body>
