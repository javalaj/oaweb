...
<oam:body>
 	<oam:header>
		...
		<h1 class="mui-title">采购合同</h1>
		<contract:seg id="mySeg" activeCellName="oaContractPurchase" />
	</oam:header>
			...
			<c:choose>
				<c:when test="${page.list.size()>0}">
					<c:forEach items="${page.list}" var="oaContractPurchase" varStatus="status">
						<oam:listViewCell onclick="location.href='${mctx}/oa/contract/oaContractPurchase/form?pageType=view&id=${oaContractPurchase.id}'">
							<p><font color="#1a81d1">
	...
	<contract:menu hideMenuCellName="oaContractPurchase" id="topPopover"/>
</oam:body>
