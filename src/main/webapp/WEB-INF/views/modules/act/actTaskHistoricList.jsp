...
<body class="gray-bg">
	<%-- <ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/act/task/tosend/">已发任务</a></li>
...
<div class="ibox-title">
	...
	<form:form id="searchForm" modelAttribute="act"
		action="${ctx}/act/task/historic/" method="post"
		class="form-inline">
	...
	<%-- 	<button class="btn btn-white btn-sm " data-toggle="tooltip"
						data-placement="left" onclick="location.href='${ctx}/act/task/tosend/'" title="已办任务">
						...
						<c:when test="${procDef.name == '周计划评价'}">
							...
							<c:when test="${procDef.name=='招待费报销申请流程'}">
							   <a href="${ctx}/oa/entertainmentr/oaEntertainmentReimbursement/view?sameid=${task.description}">详情
								...
								<c:when test="${procDef.name=='签报流程'}">
							   <a href="${ctx}/oa/sign/oaSign/ProcInsIdcompletelist?procInsId=${task.processInstanceId}">详情
							   </a>
							</c:when>
							<c:otherwise>
								<a
									href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&commid=${fns:urlEncode(task.description)}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}">详情</a>
