...
<!--即时通讯结束  -->  --%>
	...
	function openWindow()
	{
		...
		$.ajax({ 
			type: "POST", 
			url: "${ctx}/iim/contact/open",
			data:	{},
			dataType : 'json',
			success: function(b) 
			{
				if (b)
					window.open('${ctx}/sys/user/chat');
				else
					top.layer.msg("即时通讯窗口已经打开");
			}
		});
	}
		...
		$(document).ready(function() {
		  ...
		  // 绑定菜单单击事件
			 $("#side-menu a.menu").click(function(){
		  	...
		  	$('.navbar-top-links>li').mouseover(function(){
		  		$('.navbar-top-links>li>a').css({"background":"#eff4f8"});
		  	});
...
<script src="${ctxStatic}/common/inspinia.js?v=3.2.0"></script>
	...
	$(document).ready(function() {
		 ...
		 function getNotifyNum(){
			...
			function getUnreadChatNum(){
				$.get("${ctx}/iim/chatHistory/unread?updateSession=0&t="+new Date().getTime(),function(data){
					var num = parseFloat(data);
					if (num >0){
						$("#unreadNum").html(num);
						/* 消息提醒-有消息颜色改变 */
						$('.infi-icon').css("color","#35a5e7");
						$("#unreadNum").css("background","#f25d5d");
					}else{
						$('.infi-icon').css("color","#35a5e7");
			...
			setInterval(getUnreadChatNum,1000); 
	});
            ...
            <div class="row border-bottom">
                <nav class="navbar navbar-static-top" role="navigation" style="margin-bottom:0;height:60px;">
                    ...
                    <ul class="nav navbar-top-links navbar-right">
                     <li class="dropdown divider"> <a class="dropdown-toggle count-info infi-icon J_menuItem"  onclick="openWindow();">
                                <i class="fa fa-comments" title="在线沟通"></i> <span class="label label-primary" id="unreadNum"></span>
                            </a> </li>
                    <li class="dropdown">                 
                            <a class="dropdown-toggle count-info infi-icon J_menuItem"  onclick="openTab('${ctx}/oa/oaNotify/self','消息中心',false)">
                                <i class="fa fa-bell" title="消息中心"></i> <span class="label label-primary" id="notifyNum"></span>
                            </a>
