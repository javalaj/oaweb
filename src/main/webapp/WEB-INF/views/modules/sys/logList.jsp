...
<div class="ibox-title">
	...
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post"  class="form-inline">
		...
		<div class="form-group">
			<label>操作菜单：<input id="title" name="title" type="text" maxlength="50" class="form-control input-sm" value="${log.title}"/></label>				
			<label>用户ID：<input id="createBy.id" name="createBy.id" type="text" maxlength="50" class="form-control input-sm" value="${log.createBy.id}"/></label>			
			<label>URI：<input id="requestUri" name="requestUri" type="text" maxlength="50"  class="form-control input-sm" value="${log.requestUri}"/></label>		
			<label>日期范围：&nbsp;<input id="beginDate" name="beginDate" type="text" maxlength="20"
				value="<fmt:formatDate value="${log.beginDate}" pattern="yyyy-MM-dd"/>" class="laydate-icon form-control layer-date input-sm"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false})"/>
			&nbsp;--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input id="endDate" name="endDate" type="text" maxlength="20" class="laydate-icon form-control layer-date input-sm"
				value="<fmt:formatDate value="${log.endDate}" pattern="yyyy-MM-dd"/>"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false})"/></label>
