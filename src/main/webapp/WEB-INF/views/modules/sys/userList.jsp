			...
			<div class="col-sm-12">
				<form:form id="searchForm" modelAttribute="user"
					...
					<div class="form-group">
						<label>归属公司：<sys:treeselect id="company" name="company.id"
							value="${user.company.id}" labelName="company.name"
							labelValue="${user.company.name}" title="公司"
							url="/sys/office/treeData?type=1"
							cssClass=" form-control" allowClear="true" /></label>
						
						<label>登录名：<form:input path="loginName" htmlEscape="false" maxlength="50"
							class=" form-control" /></label>
						
						<label>归属部门：<sys:treeselect id="office" name="office.id"
							...
							notAllowSelectParent="true" /></label>
						
						<label>姓名：<form:input path="name" htmlEscape="false" maxlength="50"
							class=" form-control" /></label>
			...
			<div class="col-sm-12">
				<div class="pull-left">
					<%-- <shiro:hasPermission name="sys:user:add"> --%>
						<table:addRow url="${ctx}/sys/user/form" title="用户" width="800px"
							height="600px" target="officeContent"></table:addRow>
						<!-- 增加按钮 -->
					<%-- </shiro:hasPermission> --%>
					<%-- <shiro:hasPermission name="sys:user:edit"> --%>
						<table:editRow url="${ctx}/sys/user/form" id="contentTable"
							title="用户" width="800px" height="600px" target="officeContent"></table:editRow>
						<!-- 编辑按钮 -->
					<%-- </shiro:hasPermission> --%>
					...
					<%-- <shiro:hasPermission name="sys:user:import"> --%>
						...
						<oa:fileImportForm  actionUrl="${ctx}/sys/user/import" 
												buttonText=" 导入" 
												templateUrl="${ctxStatic}/template/sys/user/userData.xlsx" 
												title="用户数据EXCEL导入" 
												id="sysImport"
		...
		<table id="contentTable"
			...
			<thead>
				<tr>
					...
					<th class="sort-column o.name">调休时长</th>
			...
			<tbody>
				<c:forEach items="${page.list}" var="user">
					<tr>
						<td><input type="checkbox" id="${user.id}" class="i-checks">
						</td>
						<td><a href="#"
						...
						<td>${user.resttime}</td>
						<td style="min-width:160px"><shiro:hasPermission name="sys:user:view">
								<a href="#"
									onclick="openDialogView('查看用户', '${ctx}/sys/user/form?id=${user.id}','800px', '650px')"
									class="btn btn-info btn-xs"><i class="fa fa-search-plus"></i>
									查看</a>
							</shiro:hasPermission> <shiro:hasPermission name="sys:user:edit">
								<a href="#"
									onclick="openDialog('修改用户', '${ctx}/sys/user/form?id=${user.id}','800px', '650px', 'officeContent')"
