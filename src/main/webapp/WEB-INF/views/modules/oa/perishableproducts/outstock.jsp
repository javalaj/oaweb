...
<body>
	...
	action="${ctx}/perishableproducts/perishableProducts/outstock" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>领用时间：</label>
				<input name="beginUpdateDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${perishableProducts.beginUpdateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/> - 
				<input name="endUpdateDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${perishableProducts.endUpdateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="perishableProducts">
			<tr>
				<td>
					<fmt:formatDate value="${perishableProducts.updateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
