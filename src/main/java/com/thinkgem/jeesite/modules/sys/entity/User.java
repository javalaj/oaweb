...
public class User extends DataEntity<User> {
	...
	public String getWorkYear() {
		...
		try {
			...
			int year=new Date().getYear()-indate.getYear();
			int month=(new Date().getMonth()+1+new Date().getYear()*12)- (indate.getMonth()+1+indate.getYear()*12);
			if(year>0){
				int workYear1=month;
				int workYear2= workYear1/12;
				if( workYear2>0){
					workYear=""+workYear2;
				}else{
					workYear="不满一年";	
