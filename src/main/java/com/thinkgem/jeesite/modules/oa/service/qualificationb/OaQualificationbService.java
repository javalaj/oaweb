...
public class OaQualificationbService extends CrudService<OaQualificationbDao, OaQualificationb> {
	...
	public void save(OaQualificationb oaQualificationb,String flag) {
		...
		vars.put("personnel",WorkflowUtils.getWorkflowVarValue("oa_qualificationborrowing", "personnel", Global.getDefaultUser()));// 总经理
		vars.put("flag", true);
		vars.put("next", false);
		if(oaQualificationb.getType().equals("原件")){
			vars.put("next", true);
			vars.put("register",WorkflowUtils.getWorkflowVarValue("oa_qualificationborrowing", "register", Global.getDefaultUser()));// 人事行政部审核人
		}
