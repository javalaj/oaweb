...
public class ActTaskService extends BaseService {
	...
	@Autowired
	private MyActUtilsService myActUtilsService;// 用于判断和处理已发与已办重复的情况
	...
	public List<Act> todoList(Act act) {
		...
		for (Task task : todoList) {
			...
			e.setStatus("todo");
			e.setProcDef(ProcessDefCache.get(task.getProcessDefinitionId()));
			List<HistoricActivityInstance> histIns = historyService.createHistoricActivityInstanceQuery()
			...
			for (HistoricActivityInstance historicActivityInstance : histIns) {
				if (historicActivityInstance.getAssignee() != null) {
					userLoginName = historicActivityInstance.getAssignee();
					break;
				}
			}
			if (userLoginName != null) {
				User user = UserUtils.getByLoginName(userLoginName);
				if (user != null) {
					e.setAssignee(user.getName());// 名称
					Office office = officeService.get(user.getOffice().getId());
					String officeName = office.getName();
					if (office.getParent() != null) {
						office = office.getParent();
						if (office.getName() != null) {
							officeName = office.getName() + "-" + officeName;
						}
					}
					e.setAssigneeName(officeName);// 部门
				}
			}
			result.add(e);
		}
		...
		for (Task task : toClaimList) {
			...
			// System.out.println(historicIdentityLinksForTask.get(i)
			// .getUserId());
			// }
			// List<HistoricIdentityLink> list = historyService
			// .getHistoricIdentityLinksForProcessInstance(task
	 ...
	 * @return
	 */
	public Page<Act> todoAllList(Page<Act> page,Act act) {
		List<Act> result = new ArrayList<Act>();

		// =============== 已经签收的任务 ===============
		TaskQuery todoTaskQuery = taskService.createTaskQuery().active().includeProcessVariables()
				.orderByTaskCreateTime().desc();
		...
		todoTaskQuery.taskAssigneeLikeIgnoreCase(getPingYin(act.getAssigneeName()));
			
		}
		
		page.setCount(todoTaskQuery.count());
		// 查询列表
...
//		List<Task> todoList = todoTaskQuery.list();
		for (Task task : todoList) {
			...
			e.setTransactor(UserUtils.getByLoginName(task.getAssignee()).getName());
			}
			e.setVars(task.getProcessVariables());
			// e.setTaskVars(task.getTaskLocalVariables());
			// System.out.println(task.getId()+" = "+task.getProcessVariables()
	...
	public Page<Act> historicList(Page<Act> page, Act act) {
		...
		for (HistoricTaskInstance histTask : histList) {
			...
			HistoricTaskInstance task= historyService.createHistoricTaskInstanceQuery().taskId(histTask.getId())
			    .includeProcessVariables().singleResult();
			
			e.setHistTask(task);
			e.setVars(task.getProcessVariables());
			// e.setTaskVars(histTask.getTaskLocalVariables());
			// System.out.println(histTask.getId()+" =
			// "+histTask.getProcessVariables()
			...
			e.setStatus("finish");
			List<HistoricActivityInstance> histIns = historyService.createHistoricActivityInstanceQuery()
					.processInstanceId(histTask.getProcessInstanceId()).orderByHistoricActivityInstanceStartTime().asc()
					.orderByHistoricActivityInstanceEndTime().asc().list();
			String userLoginName = null;
			for (HistoricActivityInstance historicActivityInstance : histIns) {
				if (historicActivityInstance.getAssignee() != null) {
	 ...
	 * @return
	 */
	public Page<Act> tosendList2(Page<Act> page, Act act) {
		String userId = UserUtils.getUser().getLoginName();// ObjectUtils.toString(UserUtils.getUser().getId());
		List<HistoricProcessInstance> histTaskQuery = historyService.createHistoricProcessInstanceQuery()
				.startedBy(userId).list();// finished--> 完成的流程；  unfinish ---> 还在运行中的流程;
		List<Act> actList = Lists.newArrayList();
		for (int i = 0; i < histTaskQuery.size(); i++) {
			HistoricProcessInstanceEntity a = (HistoricProcessInstanceEntity) histTaskQuery.get(i);
			HistoricTaskInstanceQuery htq = historyService.createHistoricTaskInstanceQuery()
					.processInstanceId(a.getProcessInstanceId()).includeProcessVariables().orderByTaskCreateTime()
					.asc();
			Act e = new Act();
			e.setHistTask(htq.list().get(0));
			e.setVars(htq.list().get(0).getProcessVariables());
			e.setProcDef(ProcessDefCache.get(histTaskQuery.get(i).getProcessDefinitionId()));
			e.setStatus("finish");
			List<HistoricActivityInstance> histIns = historyService.createHistoricActivityInstanceQuery()
					.processInstanceId(a.getProcessInstanceId()).orderByHistoricActivityInstanceStartTime().asc()
					.orderByHistoricActivityInstanceEndTime().asc().list();
			String userLoginName = null;
			for (HistoricActivityInstance historicActivityInstance : histIns) {
				if (historicActivityInstance.getAssignee() != null) {
		...
		page.setList(actList);
		page.setCount(histTaskQuery.size());
		act.setPage(page);
		return page;
		// // 设置查询条件
		// if (StringUtils.isNotBlank(act.getProcDefKey())) {
		...
		// histTaskQuery.listPage(page.getFirstResult(), page.getMaxResults());
		// // 查询总数
		...
		// if
		// (histTask.getProcessInstanceId().equals(actList.get(i).getHistTask().getProcessInstanceId()))
		// {
		// flag2 = false;
		// break;
		// }
		// }
		// if (flag2) {
		// List<HistoricActivityInstance> histIns =
		// historyService.createHistoricActivityInstanceQuery()
		// .processInstanceId(histTask.getProcessInstanceId()).orderByHistoricActivityInstanceStartTime()
		...
		// flag = false;
		// break;
		// } else if (historicActivityInstance.getAssignee() != null) {
		// break;
		// }
		// }
		// }
		...
		// String userLoginName = histTask.getAssignee();
		// User user = UserUtils.getByLoginName(userLoginName);
		// if (user != null) {
		// e.setAssignee(user.getName());// 名称
		// Office office = officeService.get(user.getOffice().getId());
		...
		// .taskAssignee(UserUtils.get(act.getCurrentUser().getId()).getLoginName())
		// // 指定办理人
		// .finished() // 查询已经完成的任务
		// .list();
		//
	...
	public Page<Act> tosendList(Page<Act> page, Act act) {
		String userId = UserUtils.getUser().getLoginName();// ObjectUtils.toString(UserUtils.getUser().getId());

		HistoricProcessInstanceQuery histProcessQuery = historyService.createHistoricProcessInstanceQuery()
				// .includeProcessVariables()
				.startedBy(userId).orderByProcessInstanceStartTime().desc();

		// 设置查询条件
		if (StringUtils.isNotBlank(act.getProcDefKey())) {
			histProcessQuery.processDefinitionKey(act.getProcDefKey());
		}
		if (act.getBeginDate() != null) {
			histProcessQuery.startedAfter(act.getBeginDate());
		}
		if (act.getEndDate() != null) {
			histProcessQuery.startedBefore(act.getEndDate());
		}

		page.setCount(histProcessQuery.count());

		List<HistoricProcessInstance> processList = histProcessQuery.listPage(page.getFirstResult(),
				page.getMaxResults());
		...
		for (HistoricProcessInstance process : processList) {
			...
			List<HistoricTaskInstance> taskList = historyService.createHistoricTaskInstanceQuery()
					.processInstanceId(process.getId()).taskAssignee(userId).includeProcessVariables()
	 ...
	 * @return
	 */
	public Page<Act> tosearchList(Page<Act> page, Act act) {
		String userId = UserUtils.getUser().getLoginName();// ObjectUtils.toString(UserUtils.getUser().getId());
		...
		// 全部正在进行的
		if ("".equals(act.getFlag()) || act.getFlag() == null) {
			boolean flag = false;
			for (HistoricTaskInstance histTask : histList) {
				flag = false;
				// 条件：流程开始时间的最小值
				if (act.getBeginDate() != null && act.getBeginDate().getTime() > histTask.getStartTime().getTime()) {
					continue;
				}
				// 条件：流程开始时间的最大值
				if (act.getEndDate() != null && act.getEndDate().getTime() < histTask.getStartTime().getTime()) {
					continue;
				}
				// 条件：任务的编号包含于
				if (act.getFlag() != null && histTask.getProcessInstanceId().indexOf(act.getFlag()) == -1) {
					continue;
				}
				for (int i = 0; i < actList.size(); i++) {
					if (histTask.getProcessInstanceId().equals(actList.get(i).getHistTask().getProcessInstanceId())) {
						flag = true;
						break;
					}
				}
				...
				List<Task> list = taskService.createTaskQuery().processInstanceId(histTask.getProcessInstanceId())
						.active().list();
				Act e = new Act();

				if (list.size() > 0) {
					e.setHistTask(histTask);
					e.setTask(list.get(0));

					e.setVars(histTask.getProcessVariables());
					// 条件：标题名称包含于
					if (act.getTaskName() != null && !"".equals(act.getTaskName())) {
						if (e.getVars().getMap().get("title") != null
								&& e.getVars().getMap().get("title").toString().indexOf(act.getTaskName()) == -1) {
							continue;
						} else if (e.getVars().getMap().get("title") == null
								&& histTask.getId().indexOf(act.getTaskName()) == -1) {
							continue;

						}
					}

					e.setProcDef(ProcessDefCache.get(list.get(0).getProcessDefinitionId()));
					e.setStatus("finish");
					List<HistoricActivityInstance> histIns = historyService.createHistoricActivityInstanceQuery()
					...
					taskEntity.setCreateTime(histTask.getStartTime());
					// 条件：标题名称包含于
					// if (act.getTaskName() != null
					// && !"".equals(act.getTaskName())
					// && histTask.getProcessVariables().get("title")
					// .toString().indexOf(act.getTaskName()) == -1) {
					// continue;
					// }
					e.setTask(taskEntity);
					e.setVars(histTask.getProcessVariables());
					// 条件：标题名称包含于
					if (act.getTaskName() != null && !"".equals(act.getTaskName())) {
						if (e.getVars().getMap().get("title") != null
				...
				// 条件：申请人名称包含于
				if (act.getAssignee() != null && !"".equals(act.getAssignee())
						&& (e.getAssignee() == null || e.getAssignee().indexOf(act.getAssignee()) == -1)) {
					continue;
				}
				// 条件：部门名称包含于
				if (act.getAssigneeName() != null && !"".equals(act.getAssigneeName())
						&& (e.getAssigneeName() == null || e.getAssigneeName().indexOf(act.getAssigneeName()) == -1)) {
					continue;
				}
				...
				if (act.getTaskName() != null && !"".equals(act.getTaskName())) {
					if (e.getVars().getMap().get("title") != null
						...
						continue;
							...
							&& task.getId().indexOf(act.getTaskName()) == -1) {
						continue;
		...
		} else if ("todo".equals(act.getFlag())) {
			...
			for (HistoricTaskInstance histTask : histList) {
				...
				if (list.size() > 0) {
					...
					// 条件：部门名称包含于
					if (act.getAssigneeName() != null && !"".equals(act.getAssigneeName())
							&& (e.getAssigneeName() == null
									|| e.getAssigneeName().indexOf(act.getAssigneeName()) == -1)) {
						continue;
					}
					actList.add(e);
				}
		...
		} else if ("already".equals(act.getFlag())) {
		...
		// Comparator<Act> comparator = new Comparator<Act>() {
		// public int compare(Act s1, Act s2) {
		// // 先排年龄
		// if (s1.getBeginDate() != s2.getBeginDate()) {
		// return (int) (s1.getBeginDate().getTime() - s2
	...
	public void complete(String taskId, String procInsId, String comment, String title, Map<String, Object> vars) {
		...
		// 用于判断和处理已发与已办重复的情况
		// myActUtilsService.judgeAndDo(procInsId);
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public void jumpTask(String procInsId, String currentTaskId, String targetTaskDefinitionKey,
	...
	public ActivityImpl[] insertTasksAfter(String procDefId, String procInsId, String targetTaskDefinitionKey,
		...
		ActivityImpl prototypeActivity = ProcessDefUtils.getActivity(processEngine, processDefinition.getId(),
				targetTaskDefinitionKey);
		return cloneAndMakeChain(processDefinition, procInsId, targetTaskDefinitionKey,
				prototypeActivity.getOutgoingTransitions().get(0).getDestination().getId(), variables, newAssignees);
	}

	/**
	 * 前加签
	 */
	public ActivityImpl[] insertTasksBefore(String procDefId, String procInsId, String targetTaskDefinitionKey,
			Map<String, Object> variables, String... assignees) {
		ProcessDefinitionEntity procDef = (ProcessDefinitionEntity) repositoryService.getProcessDefinition(procDefId);
		return cloneAndMakeChain(procDef, procInsId, targetTaskDefinitionKey, targetTaskDefinitionKey, variables,
				assignees);
	}
	 ...
	 * 分裂某节点为多实例节点
	 */
	public ActivityImpl splitTask(String procDefId, String procInsId, String targetTaskDefinitionKey,
	...
	@SuppressWarnings("unchecked")
	public ActivityImpl splitTask(String procDefId, String procInsId, String targetTaskDefinitionKey,
			Map<String, Object> variables, boolean isSequential, String... assignees) {
	...
	@SuppressWarnings("unchecked")
	private ActivityImpl[] cloneAndMakeChain(ProcessDefinitionEntity procDef, String procInsId,
			String prototypeActivityId, String nextActivityId, Map<String, Object> variables, String... assignees) {
		SimpleRuntimeActivityDefinitionEntity info = new SimpleRuntimeActivityDefinitionEntity();
	...
	public InputStream tracePhoto(String processDefinitionId, String executionId) {
		...
		// activeActivityIds);
		return processEngine.getProcessEngineConfiguration().getProcessDiagramGenerator().generateDiagram(bpmnModel,
				"png", activeActivityIds, Collections.<String>emptyList(),
