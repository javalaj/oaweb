	...
	<div class="mui-content">
		...
		<div class="divcontent">
			<form action="${mctx}/oa/oaOutwork/dealExam3" method="post"
				...
				<input type="hidden" name="sign" value="1"> <input
				...
				<input type="hidden" value="${oaOutwork.scheduleProgram}"
					...
					type="hidden" value="${oaOutwork.examtext2}" name="examtext2" />
				...
				<ul class="mui-table-view divcontent">
					...
					<li class="mui-table-view-cell"><span> 确定结束时间：</span> <input
						readonly="readonly" class="inputs mui-datetime" type="text"
						name="realEndTime"
						value='<fmt:formatDate value="${oaOutwork.realEndTime}" pattern="yyyy-MM-dd HH:mm"/>'></li>
					<li class="mui-table-view-cell"><span> 出差时长：</span> <input
						class="inputs" type="text"  name="time"
						value="${oaOutwork.time}"></li>
				</ul>
	...
	<script type="text/javascript">
		...
		function judgetime() {
			...
			if ($("input[name=realEndTime]").val() == "") {
				$("input[name=realEndTime]").addClass("required");
				judge = false;
			} else {
				$("input[name=realEndTime]").removeClass("required");
			}
			//出差时长
			if ($("input[name=time]").val() == "") {
				$("input[name=time]").addClass("required");
				judge = false;
			} else {
				$("input[name=time]").removeClass("required");
			}
			return judge;
		}
		...
		function tijiao2() {
			if (judgetime()) {
				...
				var date2 = new Date($("input[name=realEndTime]").val());
				if (date2 <= date1) {
					new_alert("确定归来时间不得小于开始时间！", " ");
					return 0;
				}
