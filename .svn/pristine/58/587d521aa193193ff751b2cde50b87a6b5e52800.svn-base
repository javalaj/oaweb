//
function method_date() {
	// var optionsJson =
	// this.getAttribute('data-options') ||
	// '{}';
	var now = $(this)[0];
	var valuestr = now.value;
	var classstr = now.getAttribute("class");
	if (classstr.indexOf("mui-datetime") >= 0) {
		var optionsJson = '{"value":"' + valuestr + '"}';
	} else if (classstr.indexOf("mui-time") >= 0) {
		valuestr = "2017-04-06 " + valuestr;
		var optionsJson = '{"type":"time","value":"' + valuestr + '"}';
	} else {
		var optionsJson = '{"type":"date","value":"' + valuestr + '"}';
	}
