package com.thinkgem.jeesite.common.tag;
	...
	private static String getChildOfTree(Menu parent, int level) {
		...
		if (level > 0) {//level 为0是功能菜单
			...
			if (parent.getHref() != null && parent.getHref().length() > 0) {
				...
				if(parent.getHref().endsWith(".html")&&!parent.getHref().endsWith("ckfinder.html")){//如果是静态资源并且不是ckfinder.html，直接访问不加adminPath
					...
					href = context.getContextPath() + com.thinkgem.jeesite.common.config.Global.getAdminPath()
					+ parent.getHref();
				}
			}
		}
		...
		if (parent.hasChildren()) {
			...
			menuString
					...
					+ "\"><i class=\"iconfont "+parent.getIcon()+"\"></i> <span class=\"nav-label\">"
							+ parent.getName()
			...
			menuString.append("<a class=\"J_menuItem\"  title=\"" + parent.getName()
					+ "\"   href=\"" + href
					+ "\" ><i class=\"iconfont "+parent.getIcon()+"\"></i> <span class=\"nav-label\">"+parent.getName()+"</span></a>");
		}
		if (level > 0) {
			menuString.append("</li>");
		}
