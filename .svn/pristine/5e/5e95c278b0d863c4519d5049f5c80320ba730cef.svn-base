...
<c:choose>
	<c:when test="${isMobile == 'true'}">
		<c:if test="${not empty procInsId}">
			<button type="button" data-loading-text="撤回中"
				...
				onclick='window.location.href="${ctx}/oa/flowback/oaFlowBack/saveStaff?isMobile=true&id=${procInsId}";'>
				<span>撤 回</span>
			</button>
		</c:if>
	...
	<c:otherwise>
		<c:if test="${not empty procInsId}">&nbsp;<input
				...
				onclick='window.location.href = "${ctx}/oa/flowback/oaFlowBack/saveStaff?isMobile=false&id=${procInsId}";' />
		</c:if>
		<script>
			$(function() {
				if('${message}' != ""){
