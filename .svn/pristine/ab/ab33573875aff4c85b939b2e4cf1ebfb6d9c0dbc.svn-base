...
<body class="mui-ios mui-ios-9 mui-ios-9-1" style="background-color: #f7f7f7;">
 	<oam:header>
		...
		<h1 class="mui-title">共享文档</h1>
		<a id="menu" class="mui-action-menu mui-icon mui-icon-bars mui-pull-right" href="#topPopover"></a>
	</oam:header>
	
	<oam:content>
		<oam:searchWidget cancelSelector="#cancelButton" paddingTop="44px">
			<oam:form id="searchForm" modelAttribute="oaDoc" action="${mctx}/oa/doc/oaDoc/publicList" method="post">
			...
			<c:choose>
				<c:when test="${page.list.size()>0}">
					<c:forEach items="${page.list}" var="oaDoc" varStatus="status">
						<li class="mui-table-view-cell" onclick="location.href='${mctx}/oa/doc/oaDoc/form?id=${oaDoc.id}&prePage=/oa/doc/oaDoc/publicList'">
							<p>
