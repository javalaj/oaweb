							...
							<c:when test="${empty messageMap}"><!--消息分列表 -->
								<c:forEach items="${page.list}" var="oaNotify">
									<li class="mui-table-view-cell mui-media">
										<a href="${ctx}/m/oa/oaNotify/${requestScope.oaNotify.self?'view':'form'}?id=${oaNotify.id}&oaNotify.Mess=true&pageNo=${page.pageNo}&pageSize=${page.pageSize}">
											<div class="msg-icon">
											...
											<a href="${ctx}/m/oa/oaNotify/message?oaNotify.type=${message.key}">
												<div class="msg-icon">
													<img class="mui-media-object mui-pull-left" 
														...
														alt="${fns:getDictLabel(message.key, 'oa_notify_type', '')}通知" />
													<c:if test="${message.value > 0}">
														<span class="mui-badge">${message.value}</span>
													</c:if>
												</div>
												<div class="mui-media-body">
													<span class="msg-type">${fns:getDictLabel(message.key, 'oa_notify_type', '')}通知</span>
												</div>
											</a>
										</li>
									</c:if>
