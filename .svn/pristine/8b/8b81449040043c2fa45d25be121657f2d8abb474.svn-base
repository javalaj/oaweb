...
public class OaPcSubsidyService extends CrudService<OaPcSubsidyDao, OaPcSubsidy> {
	private static final String PROC_DEF_KEY = "oa_pc_subsidy";// 流程定义标示
	@Autowired
	...
	public void save(OaPcSubsidy oaPcSubsidy, String flag) {
		...
		vars.put("fixedAssetManager",
				WorkflowUtils.getWorkflowVarValue(PROC_DEF_KEY, "fixedAssetManager", Global.getDefaultUser()));// 固定资产管理员登陆名字
		...
		if (StringUtils.isBlank(oaPcSubsidy.getProcInsId())) {
			...
			if (list.size() > 0) {
				...
				// 流程撤回操作数据设置
				oaFlowBackService.saveINIT(oaPcSubsidy.getProcInsId(), list.get(0),
						"/oa/oaPcSubsidy/form?flag=view&id=" + oaPcSubsidy.getId());
			}
		} else {
			...
			if (list.size() > 0) {
				...
				if (flag.equals("1")) {
					...
					// 流程撤回操作数据设置
					oaFlowBackService.saveNOW(oaPcSubsidy.getProcInsId(), list.get(0));
				}
			}
	...
	public void dealExam(OaPcSubsidy oaPcSubsidy, String exam, String sign) {
		...
		if (list.size() > 0) {
			...
			if ((!sign.equals("1")) && exam.equals("0")) {
				// 流程撤回操作数据设置
				oaFlowBackService.saveNULL(oaPcSubsidy.getProcInsId());
			} else {
				// 流程撤回操作数据设置
