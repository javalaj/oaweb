		...
		$(document).ready(function() {
			...
			$("#btnSubmit").click(function() {
				...
				for (var i = 0; i < isagree.length; i++) {
					isagree[i].value=exam;
				}
				var examtext = $('input[name="text"]').val();
				var text=document.getElementsByName("examtext");
 				for (var i = 0; i < text.length; i++) {
					text[i].value=examtext;
 				}
				//text.value=examtext;
// 				$("#agree").val(exam);				
				submitUserList_5();
...
<%-- 		<input  value="${listOaEnter}" name="listOaEnter"/> --%>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
			<tr>
				<td colspan="8" class="head">招 待 费 报 销 审 批 单</td>
			</tr>
			<tr>
				<td rowspan="2">日期</td>
			...
			<c:forEach var="oaEnter" items="${listOaEnter}">
				<tr>
					...
					<input type="hidden" name="isagree" id="agree"/>
					<input type="hidden" name="examtext" id="examt"/>
					...
					<input type="hidden" name="createBy" value="${oaEnter.createBy.name}"/>
					</td>					
				</tr>
				</c:forEach>
				<tr>
				...
				<tr>
					<td colspan="2">上级领导审核意见</td>
					<td colspan="6"><input type="text" name="text" class="form-control"/></td>
				</tr>					
			</tbody>
		</table>
	...
	<script type="text/javascript">
		...
		function submitUserList_5() {
			...
			$.ajax({
				url : "${ctx}/oa/entertainmentr/oaEntertainmentReimbursement/dealExam",
				type : "POST",
				...
				success : function(data) {
					...
					if (data == "1") {
						...
						$("#messageBox").show();
						window.location.href = '${ctx}/oa/entertainmentr/oaEntertainmentReimbursement/exam';
					} else {
						top.$.jBox.tip.mess = 1;
