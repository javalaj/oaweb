...
<body>
	...
	<div class="row" >
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/profileOutState" method="post" class="form-inline ">
...
<%-- 	    <button class="btn btn-white btn-sm " data-toggle="tooltip"
	...
	<div style="width:100%;max-height:500px;overflow-x:auto;">	
	<table id="contentTable" class="table table-striped table-bordered  table-hover table-condensed  dataTables-example dataTable no-footer">
		...
		<c:forEach items="${page.list}" var="user"  varStatus="status">
			<tr>
				<tr>
				<td><input
											type="checkbox" name="idCheck" value="${user.id}" id="${user.id}" class="i-checks"/></td>								
				...
				<td  class="hideInfo"><fmt:formatDate value="${user.contractStart}" pattern="yyyy-MM-dd"/>至<fmt:formatDate value="${user.contractEnd}" pattern="yyyy-MM-dd"/></td>
