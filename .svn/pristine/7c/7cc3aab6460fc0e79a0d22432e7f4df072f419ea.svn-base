...
<head>
	...
	<script type="text/javascript">		
		...
		function uiResize() {
			...
			//内容的高度,系数不可调
			$("#tableContent").height($("#mainContent").height()-110);
		}
	</script>
</head>
<body class="gray-bg">
...
<div class="ibox-title">
	...
	<!-- 工具栏 -->
		...
		<div style="width: 100%;overflow: auto;margin-bottom: 6px;" id="tableContent">
			<table class="table table-striped table-bordered table-hover dataTables-example dataTable no-footer myoa-table-width280">
				...
				<tbody>
					<c:forEach items="${page.list}" var="procIns">
						<tr>
							<td title="${procIns.id}">${procIns.id}</td>
							<td title="${procIns.processInstanceId}">${procIns.processInstanceId}</td>
							<td title="${procIns.processDefinitionId}">${procIns.processDefinitionId}</td>
							<td title="${procIns.activityId}">${procIns.activityId}</td>
							<td title="${procIns.suspended}">${procIns.suspended}</td>
							<td>
