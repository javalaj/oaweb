...
function openTab(url,title, isNew){//isNew 为true时，打开一个新的选项卡；为false时，如果选项卡不存在，打开一个新的选项卡，如果已经存在，使已经存在的选项卡变为活跃状态。
    ...
    if(start>0){
   	var newurl=dataUrl.substring(0,start);
    }
...
//    });
    ...
    if(!isNew){
		    ...
		    top.$('.J_menuTab').each(function () {
		        ...
		        if (top.$(this).data('id') == newurl) {// 选项卡已存在，激活。	
		        	 var target = $('.J_iframe[data-id="' + $(this).data('id') + '"]');
	                    if (target.attr('src') !=dataUrl ) {
	                    	//显示loading提示
	                        //var loading = layer.load();
    ...
    if(isNew || flag){//isNew为true，打开一个新的选项卡； flag为true，选项卡不存在，打开一个新的选项卡。
	        var str = '<a href="javascript:;" class="active J_menuTab" data-id="' + newurl + '">' + menuName + ' <i class="fa fa-times-circle"></i></a>';
	        top.$('.J_menuTab').removeClass('active');
	
	        // 添加选项卡对应的iframe
	        var str1 = '<iframe class="J_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + newurl + '" seamless></iframe>';
