	...
	<div class="wrapper wrapper-content">
		<div class="ibox">
    		...
    		<div class="ibox-content">
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="oaProjectApproval" action="${ctx}/oa/project/oaProjectApproval/" method="post" class="form-inline">
							...
							<oa:searchGroup id="searchGroup">
								...
								<label>审批状态：
									<form:select path="status" class="selectpicker " data-style="btn-inverse" data-live-search="true" id="status">
										<form:option value="" label="全部"/>
										<form:options items="${fns:getDictList('ActEntity_ACT_STATUS')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
									</form:select>
								</label>
							</oa:searchGroup>
						</form:form>
							...
							<div class="col-sm-12">
								<div class="pull-left">
									<shiro:hasPermission name="oa:project:oaProjectApproval:edit">
										<a href="${ctx}/oa/project/oaProjectApproval/form"
						...
						<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
							...
							<c:forEach items="${page.list}" var="oaProjectApproval" varStatus="status">
								<tr>
									...
									<td>
										${fns:getDictLabel(oaProjectApproval.status, 'ActEntity_ACT_STATUS', '')}
									</td>
									<td>
										<shiro:hasPermission name="oa:project:oaProjectApproval:edit">
						    				<a href="${ctx}/oa/project/oaProjectApproval/form?id=${oaProjectApproval.id}" 
													class="btn ${oaProjectApproval.status eq '0'?'btn-success':'btn-info'} btn-xs"><i class="fa ${oaProjectApproval.status eq '0'?'fa-edit':'fa-search-plus'}"></i>${oaProjectApproval.status eq '0'?'修改':'查看'}</a>
										</shiro:hasPermission>
									</td>
								</tr>
							</c:forEach>
