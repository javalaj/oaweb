...
<%-- 		<input  value="${listOaEnter}" name="listOaEnter"/> --%>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
			...
			<c:forEach var="oaEnter" items="${listOaEnter}">
				<tr>
					...
					<input type="hidden" name="isagree1" value="" id="isagree1"/>
					<input type="hidden" name="examtext1" value=""/>
				...
				<tr>
					<td colspan="2">总经理审核意见</td>
					<td colspan="6"><input type="text" name="examtext" class="form-control"/></td>
				</tr>					
			</tbody>
		</table>
	...
	<script type="text/javascript">
		...
		function submitUserList_5() {			
			...
			$.ajax({
						url : "${ctx}/oa/entertainmentr/oaEntertainmentReimbursement/dealExam1",
						type : "POST",
						...
						success : function(data) {
							...
							if (data == "1") {
								...
								$("#messageBox").show();
								window.location.href = '${ctx}/oa/entertainmentr/oaEntertainmentReimbursement/exam1';
							} else {
