...
public class PerishableSerialService extends CrudService<PerishableSerialDao, PerishableSerial> {
	 ...
	 */
	@Transactional(readOnly = false)
	public PerishableSerial saveForAct(String[] perishableId,Integer[] amount, String remarks)
	{
		PerishableSerial perishableSerial = new PerishableSerial();
		
		String title = UserUtils.getUser().getName() + "的易耗品申请";
		
		perishableSerial.setTitle(title);
		
		perishableSerial.setRemarks(remarks);
		
		save(perishableSerial);
		...
		for (int i = 0;i < perishableId.length;i++)
		{
			...
			apply.setSerialId(perishableSerial.getId());
			...
			perishableApplyService.save(apply);
		...
		startProcess(perishableSerial,vars);
		
		return perishableSerial;
	}
	 ...
	 * @param vars
	 */
	public void startProcess(PerishableSerial perishableSerial, Map<String, Object> vars)
	{
		String title = titleHandle(perishableSerial,PerishableSerial.WORKFLOW_NAME);
		
		// 第一次申请，启动工作流
		String procInsId = actTaskService.startProcess(PerishableSerial.PROC_DEF_KEY, PerishableSerial.BUSINESS_TABLE, perishableSerial.getId(), title, vars);
		// 线程暂停两秒
		try 
		{
			Thread.sleep(2000);
		...
		if (list.size() > 0) 
		{
			String loginName = WorkflowUtils.getWorkflowVarValue(PerishableSerial.PROC_DEF_KEY, "admin", "");
		
			examMsg(perishableSerial, PerishableSerial.WORKFLOW_NAME, PerishableSerial.WORKFLOW_URL_EXAM, loginName);
	...
	{
		PerishableProducts obj = perishableProductsService.get(perishableId);
	...
	public Map<String, Object> initVars()
	{
		...
		vars.put("admin", WorkflowUtils.getWorkflowVarValue(PerishableSerial.PROC_DEF_KEY, "admin", ""));
		
		vars.put("staff", WorkflowUtils.getWorkflowVarValue(PerishableSerial.PROC_DEF_KEY, "staff", ""));
		
		vars.put("file", WorkflowUtils.getWorkflowVarValue(PerishableSerial.PROC_DEF_KEY, "file", ""));
	...
	public List<PerishableApply> getApplyList(List<PerishableApply> list)
	{
		...
		if (list.size() > 0)
			for (PerishableApply perishableApply : list) 
				temp.add(perishableApplyService.get(perishableApply.getId()));
	...
	public void exam(boolean b, String[] ids, String[] amount,PerishableSerial perishableSerial)
	{
		...
		List<Task> list = actTaskService.getProcessEngine().getTaskService().createTaskQuery().processInstanceId(perishableSerial.getProcInsId())
		...
		if (b)
		{
			...
			{
				PerishableProducts temp = perishableProductsService.get(ids[i]);
				//更新库存
				temp.setAmount(temp.getAmount() + Integer.parseInt(amount[i]));
				
				perishableProductsService.save(temp);
			}
			
			if (list.size() > 0) 
			{
				String workflowTitle = retreatHandle(perishableSerial, PerishableSerial.WORKFLOW_NAME, PerishableSerial.WORKFLOW_URL_FORM);
				
				vars.put("flag", false);
				
				vars.put("apply", UserUtils.get(perishableSerial.getCreateBy().getId()).getLoginName());
				
				actTaskService.complete(list.get(0).getId(), perishableSerial.getProcInsId(), "【退回】", workflowTitle, vars);
			}
		}
		else
		{
			...
			if (list.size() > 0) 
			{
				...
				actTaskService.complete(list.get(0).getId(), perishableSerial.getProcInsId(), "【同意】", title, vars);
			}
		}
	}
	...
	public void application(PerishableSerial perishableSerial)
	{
		...
		if (list.size() > 0) 
		{
			...
			actTaskService.complete(list.get(0).getId(), perishableSerial.getProcInsId(), "【领用完成】", title, vars);
			
			completedHandle(perishableSerial, PerishableSerial.WORKFLOW_NAME, PerishableSerial.WORKFLOW_URL_EXAM);
		}
	}
	
	@Autowired
	private PerishableApplyService perishableApplyService;
